...
本研究旨在完成平面標定板之單眼相機校正，並以血汗手刻程式與OpenCV call function 進行比較。研究問題聚焦於：在不直接呼叫內建校正解算器的前提下，能否僅依賴棋盤格角點對應，穩定估測相機內參矩陣與多視角外參，並以重投影誤差驗證其可用性。
...
此外，實驗流程支援替換資料集（data 與 my_data），以觀察不同拍攝條件下估測品質與方法一致性。
